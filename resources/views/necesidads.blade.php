<x-app-layout>
    <x-container class="pt-5">
        <h1 class="text-2xl font-bold text-gray-700 mb-2">
            Totalidad de necesidades registradas
        </h1>
        @if ($necesidadzCargadas->count())
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 card bg-gray-200 gap-6">

                @foreach ($necesidadzCargadas as $necesidad)
                    <article class="bg-white shadow rounded overflow-hidden">
                        <img src="{{ Storage::url($necesidad->cuarto['photo_1']) }}"
                        ...
                        <div class="p-4">
                            <h1>Localidad: {{ $necesidad->cuarto->localidad->name }}</h1>
                            <h1>Ubicación: {{ $necesidad->cuarto->name }}</h1>
        ...
        @else
            <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400"
                ...
                <div>
                    <span class="font-medium">Info alert!!</span>Aun no se ha registrado ninguna necesidad.                  
                </div>
            </div>
        @endif
    </x-container>
